...
//Functio to get all IRinfo from D
  ...
  return new Promise((resolve, reject) => {
    db_con.query("SELECT Bank_ID, Bank_Type, Bank_Name, Compounding_Period, Nick_Name ,Interest_info from Bank_Master where Status = 'Active';", function (err, result) { //appropriate querry
      if (err) {
        ...
        if (debug) LOG.logDebug("successfully bankinfo fetch querry fired!\n");
        infoarr = result;
        resolve(infoarr);
      }
    });
...
function get_durs(i) {
  ...
  infoarr.forEach(element => {
    tem_obj = { bankID: '', bank: '', IR: [], banktype: '', nickname: '', compounding_period: '' };
    ...
    for (const obj of element.Interest_info) {
      // console.log(obj.From_days,obj.To_days);
...
function findIR(dur_obj) {
  ...
  dur_obj.forEach(bankinfo => {
    ...
    for (const dur of bankinfo.IR) {
      if ('SrInterest' in dur) IR = parseFloat(dur.SrInterest);
      else continue;
...
function calAPYfdtableDur(display_row_arr, i) {
  if (i > 1) { //APY Calculation needed.
    for (const [key, value] of Object.entries(display_row_arr)) {
      ...
      var rPrime = (parseFloat(value.SrInterest) * cp) / 12; //rate for one compounding period.
      ...
      value.SrAPYRate = APYRate.toFixed(2);

      delete value.APYRate;
    }
  }
  else {
    for (const [key, value] of Object.entries(display_row_arr)) {
...
async function sort_display_row(display_row_arr, id, db_con) {
  return new Promise((resolve, reject) => {
    ...
    items.sort(
      (first, second) => {
        return parseFloat(second[1].SrInterest) - parseFloat(first[1].SrInterest)
      });
    let tablename = { 1: 'Days0_45', 2: 'Days46_90', 3: 'Days91_180', 4: 'Months6_12', 5: 'Years1_2', 6: 'Years2_3', 7: 'Years3_4', 8: 'Years4_5', 9: 'Years5Plus' }
    db_con.query(`update sr_FDtable set ${tablename[id]} = default where id > ${items.length};`, function (err, result) { //appropriate querry
      if (err) {
        LOG.logError(err);
        reject();
      } else {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      let update_querry = `update sr_FDtable set ${tablename[id]} = '${json_obj}' where id = ${x};`;

      db_con.query(update_querry, function (err, result) { //appropriate query
        if (err) {
          LOG.logError(err);
          reject();
        } else {
...
async function main(db_con) {
  return new Promise(async (resolve, reject) => {
    if (debug) LOG.logDebug('srfdtable main called');
    ...
    for (const element of display_col) {
      if (debug) LOG.logDebug("During display_col SrFDtable " + j);
      ...
      if (debug) LOG.logDebug(eventually);
      j += 1;
    }
    ...
    db_con.end(function (err) {
      if (err) {
        ...
        LOG.logDebug('db con ended Srtable');
        resolve()
      }
    })
  })
